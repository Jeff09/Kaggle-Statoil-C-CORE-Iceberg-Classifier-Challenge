...
X_train_ = resize_image2(X_train)
X_test_ = resize_image2(X_test)
...
#X_train_ = resize_image(X_train, (224, 224, 3))
#X_test_ = resize_image(X_test, (224, 224, 3))
...
from keras.layers import Conv2D, MaxPooling2D, Dense, Dropout, Input, Flatten, Activation, Lambda, GlobalAveragePooling2D, GlobalMaxPooling2D
...
def getVggAngleModel():
    ...
    base_model = ResNet50(weights='imagenet', include_top=False, 
                 input_shape=X_train_.shape[1:], classes=1)    
    x = base_model.get_layer('avg_pool').output
    x = GlobalMaxPooling2D()(x)
    #base_model2 = MobileNet(weights=None, alpha=1.5,input_tensor = base_model.input,include_top=False, input_shape=X_train.shape[1:])
    
    #base_model2 = MobileNet(weights=None, alpha=1.5,input_tensor = base_model.input,include_top=False, input_shape=X_train_[0].shape)
...
preds=myAngleCV(X_train_, X_angle, X_test_)
