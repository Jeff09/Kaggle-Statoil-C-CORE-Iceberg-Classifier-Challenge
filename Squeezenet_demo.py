...
#swap tensor axis of image batches from (batch_size, row, col, ch) to (batch_size, ch, row, col)
X_train = np.transpose( X_train, (0,3,1,2))
X_test = np.transpose( X_test, (0,3,1,2))

#from keras import backend as K
#X_train = K.permute_dimensions(X_train, (0,3,1,2))
#X_test = K.permute_dimensions(X_test, (0, 3, 1, 2))
...
def getVggAngleModel():
    ...
    base_model = SqueezeNet(2, inputs=(3, 224, 224))
    
    x = base_model.get_layer('conv10').output
...
submission.to_csv('Squeezenet.csv', index=False)